...
def parse_args():
    ...
    poisson_gen_cmds = parser.add_mutually_exclusive_group()
    poisson_gen_cmds.add_argument("-M", "--multiply-all-rates-by-factor", default=1, type=float,
    ...
    poisson_gen_cmds.add_argument("-t", "--timeunit", type=int, default=60,
                        help="The number of seconds per time unit, e.g for unit 'minute' give 60, for hour 3600 etc. This only affects how the rates are interpreted. A setting of 1 means rates are 'per second', a setting of 60 means rates are 'per minute', a setting of 3600 means rates are 'per hour'. Output is always in format HH:MM:SS:SSSSSS, as is input (when using -f).")

    file_splitting_cmds = parser.add_mutually_exclusive_group()
    file_splitting_cmds.add_argument("-f", "--trace-input-file", type=str, default=None,
