...
if llsf:
    for i in placementDict:
        if len(placementDict[i][1])>1:
            singleInput = [x for x in placementDict[i][2] if len(x) == 1][0]
            placementDict[i] = (placementDict[i][0], singleInput, placementDict[i][2])
...
def processingRules(i):  
    ...
    predicate_constraints  = getPredicateConstraints_adjust(mycombination[0], mycombination[1])
    #predicate_constraints = constraintDict[projection][2]
    print(name, input_1, input_2, predicate_constraints)
    ...
    return name, output_selection, input_1, input_2, selectivity,constraintDict[projection][0],id_constraints,predicate_checks,projection,predicate_constraints
...
def main():
    ...
    for node in range(len(mynw)):
        ...
        query_name, output_selection, input_1, input_2, selectivity, sequence_constraints, id_constraints,predicate_checks, query,predicateConstraints = processingRules(node)
