...
class Tree():
    ...
    def __eq__(self, other): 
        if str(self) == str(other):
            return True
        if isinstance(other,KL) and isinstance(self,KL):
            return other.leafs() == self.leafs()
    ...
    def getsubop(self, *nodes):
            ...
            for i in sorted(subop.getnodes(), key = lambda x: subop.level(x), reverse = True):      
                ...
                if not list(i.children):
                    ...
                    l.remove(i)
                    parent.children = l
                if len(i.children) == 1 and not isinstance(i,KL):
                    if subop.is_root(i):                        
                        subop = i.children[0]                        
                    else: 
                        parent = subop.getparent(i)
    ...
    def can_be_used_combi(self, other):
        """
            ...
            for k in othersequence.keys(): 
                ...
                for v in othersequence[k]:
                    if not v in mysequence[k]:
                        return False
        if not self.hasKleene() and other.hasKleene():
            return False
        
        if self.hasKleene() and not other.hasKleene():     
            if str(self.kleene_components()[0]) in other.leafs() and set(self.leafs()) != set(other.leafs()):
                return False
        return True
    ...
    def strip_NSEQ(self):
        ...
        nodes = me.getnodes()
        if len(nodes) == 1:
            if isinstance(nodes[0], NSEQ):
                newme = SEQ()
                newme.children = copy.deepcopy(nodes[0].children)
                return newme
        else:
            for i in nodes:
    ...
    def get_negated(self):
        negated = []
        for i in self.getnodes():
            if isinstance(i, NSEQ) and len(i.children)==3:    
...
q = AND(PrimEvent('A'), SEQ(KL(PrimEvent('E')),  NSEQ(PrimEvent('B'), PrimEvent('D'))), PrimEvent('F'))
print(q.getparent((PrimEvent('E'))))
print(q.getsubop([PrimEvent('E')]))
