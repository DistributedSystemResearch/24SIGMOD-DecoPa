...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 18 22:28:05 2023
...
def getSelectivity(query):
    return totalRate(query) / (2* totalRate(placementDict[query][2][0]) * totalRate(placementDict[query][2][1]))
...
def main():
    ...
    for node in mynodes:
        ...
        for i in sorted(placementDict, key = len):
             if i in wl:
                myleafs = i.leafs()
                input1 = myleafs[0]
                input2 = myleafs[1]
                ...
                input5 = myleafs[4]        
                myqueryname = i
        # generate json fields here
        myqueries = []
        myoutputs = []
        myselectivities = []
        for i in sorted(placementDict, key = len):
            myqueries.append(i)
            myoutputs.append(i.leafs())
            myselectivities.append(getSelectivity(i))
         
         
        subquery1 = myqueries[0]   
        subquery2 = myqueries[1]     
        subquery3 = myqueries[2]     
        subquery4 = myqueries[3]     

        selectivity1 = myselectivities[0]
        selectivity2 = myselectivities[1]
        selectivity3 = myselectivities[2]
        selectivity4 = myselectivities[3]

        output_selection1 = subquery1.leafs()
        output_selection2 = subquery2.leafs()
        output_selection3 = subquery3.leafs()
        output_selection4 = subquery4.leafs()
        ...
        config = {
            ...
            "processing": {
                ...
                "predicate_checks": 1,
                "query_name": str(myqueryname),
                
                "output_selection1": output_selection1,
                "output_selection2": output_selection2,
                "output_selection3": output_selection3,
                "output_selection4": output_selection4,
                ...
                "selectivity3": selectivity3,
                "selectivity4": selectivity4,
                
                "subquery_1":str(subquery1),
                "subquery_2":str(subquery2),
                "subquery_3":str(subquery3),
                "subquery_4":str(subquery4),
