...
for nw in {1..1}; do
	...
	python generate_network.py 1 "$scaling" #new network generated and must be saved to pickle 
		...
		#for tw in 1 10 20 30; do
			...
			cd ..
			./generate_examples_twomatrices_parallel_only_citibike.sh "parallel_${scaling}" ${scaling}
			...
			cd ..
			./generate_examples_twomatrices_citibike.sh  "stateparallel_${scaling}" ${scaling}
			# move folder to directory	
			cd plans
